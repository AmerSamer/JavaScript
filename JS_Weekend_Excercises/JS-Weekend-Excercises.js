...
function basicOp(opp, val1, val2) {
    if (opp === '+')
        return val1 + val2
    if (opp === '-')
        return val1 - val2
...
function nb_year(p0, percent, inhabitants, population) {
    let result = 0
    let counter = 0
    while (result <= population) {
        result = p0 + (p0 * (percent / 100)) + inhabitants
        p0 = result
        counter++
    }
    return counter
...
function x(str) {
    ...
    let f = string[0][0].toUpperCase()
    let s = string[1][0].toUpperCase()
    return f + '.' + s
}
...
function shortest(str) {
    let string = str.split(' ');
    let shortest = string.reduce((shortestWord, currentWord) => {
...
function longest(str) {
    let string = str.split(' ');
    let longest = string.reduce((shortestWord, currentWord) => {
        return currentWord.length > shortestWord.length ? currentWord : shortestWord;
    });
    return longest;
}
...
const words = ['Monica', 'Rachel', 'Ross', 'phoebe', 'chandler', 'Joe'];
...
// - ForEach
let sumOfFriendsNamesLength = 0
words.forEach(element => {
    sumOfFriendsNamesLength += element.length
});
console.log(sumOfFriendsNamesLength);

// - Map
let a = words.map(function (x) {
